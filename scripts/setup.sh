...
# Dev environment setup for the yandex_alice provider.
set -euo pipefail
...
if [ -n "$WORKSPACE" ]; then
  ...
  LINK="$WORKSPACE/ma-server/music_assistant/providers/yandex_alice"
  TARGET="$(python3 -c "import os; print(os.path.relpath('$REPO_ROOT/provider/', '$(dirname "$LINK")'))")"
  rm -f "$LINK"
  ln -s "$TARGET" "$LINK"
  echo "Symlink: yandex_alice → $TARGET"
  ...
  echo "Provider yandex_alice linked to workspace. Activate: source $WORKSPACE/.venv/bin/activate"
else
